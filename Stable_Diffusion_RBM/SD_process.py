...
if __name__ == '__main__':
    ...
    for filename in tqdm(filenames):
        ...
        piano_roll[:4, :] = -1
        piano_roll[-4:, :] = -1
        num_jpg = piano_roll.shape[1] // (64*8)
        selected_indices = sorted(random.sample(range(num_jpg), int(num_jpg * 0.1)))
        for i in selected_indices:
            picture = piano_roll[:, i*(64*8):(i+1)*(64*8)]
            picture_raw = np.zeros((768, 512))
            count = 0
            for j in range(8):
                expand_picture = picture[:, count*64:(count+1)*64]
                picture_raw[:, j*64:(j+1)*64] = np.repeat(expand_picture, 8, axis=0)
                count += 1
            rgb_image = np.zeros((768, 512, 3), dtype='uint8')
            rgb_image[picture_raw == -1] = [127, 127, 127]  # gray
            rgb_image[picture_raw == 0] = [255, 255, 255]  # white
            rgb_image[picture_raw == 1] = [0, 0, 0]  # black
